...
RUN dnf install -y bash wget ansible unzip openssh-clients graphviz perl
...
ADD https://storage.googleapis.com/kubernetes-release/release/${K8S_VERSION}/bin/linux/amd64/kubectl /bin/kubectl
RUN echo "${K8S_SHA256SUM} /bin/kubectl" > /bin/kubectl.SHA256SUMS
RUN sha256sum -c /bin/kubectl.SHA256SUMS | egrep -e '(OK|FAILED)$'; echo $?
